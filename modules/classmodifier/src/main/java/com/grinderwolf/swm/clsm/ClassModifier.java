...
public class ClassModifier {
    ...
    public static CompletableFuture<Either<?, ?>> getFutureChunk(Object world, int x, int z) {
        if (customLoader == null || !isCustomWorld(world))
            return null;

        return CompletableFuture.supplyAsync(() -> Either.left(customLoader.getChunk(world, x, z)));
    }
