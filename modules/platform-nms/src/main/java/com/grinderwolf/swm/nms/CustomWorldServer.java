...
public class CustomWorldServer extends WorldServer {
    ...
    public CustomWorldServer(
            CraftSlimeWorld world, IWorldDataServer worldData,
        ...
        super(MinecraftServer.getServer(), MinecraftServer.getServer().executorService,
                SlimeNMSPlatform.CONVERTABLE.c(world.getName(), dimensionKey),
    ...
    ProtoChunkExtension getChunk(int x, int z) {
        ...
        if (slimeChunk instanceof NMSSlimeChunk) {
            ...
            if (slimeChunk == null) {
                ...
                chunk = new Chunk(this, pos, biomeStorage, ChunkConverter.a, blockTickList, fluidTickList, 0L, null, null);
            } else {
    ...
    private Chunk createChunk(SlimeChunk chunk) {
        ...
        BiomeStorage biomeStorage = new BiomeStorage(
                ...
                pos,
                getChunkProvider().getChunkGenerator().getWorldChunkManager(),
                biomeIntArray
        );
        ...
        for (int sectionId = 0; sectionId < chunk.getSections().length; sectionId++) {
            ...
            if (slimeSection != null) {
                ...
                section.getBlocks().a((NBTTagList) Converter.convertTag(slimeSection.palette()), slimeSection.blockStates());

                if (slimeSection.blockLight() != null)
                    lightEngine.a(EnumSkyBlock.BLOCK, SectionPosition.a(pos, sectionId), Converter.pureToNms(slimeSection.blockLight()), true);

                if (slimeSection.skyLight() != null)
                    lightEngine.a(EnumSkyBlock.SKY, SectionPosition.a(pos, sectionId), Converter.pureToNms(slimeSection.skyLight()), true);
...
//        getChunkProvider().addTicket(SWM_TICKET, pos, 33, Unit.INSTANCE);
                ...
                for (CompoundTag tag : tileEntities) {
                    ...
                    if (type.isPresent()) {
                        BlockPosition blockPosition = new BlockPosition(
                                tag.getIntValue("x").orElseThrow(),
                                tag.getIntValue("y").orElseThrow(),
                                tag.getIntValue("z").orElseThrow()
                        );
        ...
        Chunk nmsChunk = new Chunk(
                this,
                pos,
                biomeStorage,
                upgradeDataTag == null ? ChunkConverter.a : new ChunkConverter((NBTTagCompound) Converter.convertTag(upgradeDataTag)),
                blockTickList,
                fluidTickList,
                0L,
                sections,
